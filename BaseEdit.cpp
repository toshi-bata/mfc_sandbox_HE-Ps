...
{
	bool bSuccess = true;
	std::match_results<const TCHAR *> what;

	// This is a full match check, not a partial match
	bSuccess = std::regex_match (static_cast<const TCHAR *>(strText),
		what, *m_pRegEx, std::regex_constants::match_default);
	return bSuccess;
}
	...
	const bool /*bDisplayToolTip = true*/)
{
	...
	if (nChar == VK_BACK)
	{
		...
		bCharValid = IsTextValid(m_strProposedText);
	}
	// Ignore ^X, ^C and ^V
	...
	const bool bDisplayToolTip/* = true*/)
{
	...
	if (strText.GetLength ())
	{
		...
		// std::regex doesn't have match_partial like boost - so we can't perform partial matches
		bSuccess = std::regex_match (static_cast<const TCHAR *>(strText),
			what, *m_pRegEx, std::regex_constants::match_default);// | boost::match_partial);
	...
	if (m_bProcessChars)
	{
		switch (message)
		{
			case EM_REPLACESEL:
			{
				...
				bExecute = bResetValid = IsTextValid(m_strProposedText);
